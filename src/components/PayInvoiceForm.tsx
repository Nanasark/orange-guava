...
export default function BuyCryptoForm({ merchantAddress }: BuyCryptoFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const invoiceRespone = await fetch("");
      const invoice = await invoiceRespone.json();
      const response = await fetch("/api/momo-transaction/send-crypto", {
        ...
        body: JSON.stringify({
          ...
          otpcode: "",
          reference: reference,
          address: invoice.data.receiverAddress,
          merchantAddress: invoice.data.merchantAddress,
        }),
      });
      ...
      <div className="w-full max-w-md bg-white rounded-lg shadow-md p-6 border border-blue-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <p className="text-sm text-blue-600">
            You are paying {calculatedAmount.totalAmountInGHS} GHS at rate of
            5GHS per token
          </p>
          <div>
            {address ? (
